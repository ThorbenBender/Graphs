...
print(type(world.startingRoom.n_to))
...
# Fill this out
def room_direction():
    # returns an array of all the different arrays the room have
    directions = player.currentRoom.getExits()
    # create an empty dictionary
    map_piece = {}
    # loop over directions
    for direction in directions:
        # assign the direction with a question mark
        map_piece[direction] = '?'
...
def move(player, direction, traversalPath):
    # use the build in player function to move to the next room
    player.travel(direction)
    # add the direction to the new path
...
def move_toward_end(player, map, traversalPath):
    # assign player current room to current_room
    current_room = player.currentRoom
    # gets all the directions out of room_visited
    map_piece = rooms_visited[current_room.id]

    # create a empty array
    possible_direction_rooms = []
    # loop over the map_piece
    for key in map_piece:
        # check if the direction has a question mark
        if map_piece[key] == '?':
            # if it has add it to the array
            possible_direction_rooms.append(key)
    # if possible direction is empty return nothing
    ...
    # call the move function with the player, the direction you want to go to, and the traversalPath
    move(player, direction_to_go, traversalPath)

    # assign the current player room as the new player room
    new_room = player.currentRoom

    # add the direction_to_go as key with the value of the new room
    map_piece[direction_to_go] = new_room.id

    # check if currentRoom is in map
    if player.currentRoom.id in map:
        # add the current room as a direction
        map[player.currentRoom.id][opposite_direction(
            direction_to_go)] = current_room.id
    else:
        # if not in map
        # get the direction from the current_room
        new_map_piece = room_direction()
        # add the current room to the path
        new_map_piece[opposite_direction(direction_to_go)] == current_room.id
        #add new map piece to map
        map[new_room.id] = new_map_piece

    # call the function again
    move_toward_end(player, map, traversalPath)


def find_path(current_room_id, map):
    # create a queue
    q = Queue()

    # add the current_room_id to the queue
    q.enqueue([current_room_id])

    # create an empty array
    visited = []

    # create another empty array for possible paths
    possible_paths = []
    ...
    while q.size() > 0:
        ...
        # assign the path to the removed queue item
        path = q.dequeue()

        # assign the last item out of the array to the room
        room = path[-1]

        # check if room is not in visited
        if not room in visited:
            
            # if not add the room to visited
            visited.append(room)

            # loop over every direction in the room
            for direction_key in map[room]:
                # assign the neighbour to the next room
                neighbour = map[room][direction_key]
                # check if found possible path is wrong and check if possible path is smaller than paths
                if found_a_possible_path and len(possible_paths[0]) < len(path):
                    # if true break 
                    break
                # if neighbour is equal to question mark
                elif neighbour == '?':
                    # add it to possible paths
                    possible_paths.append(path)
                    # turn found_a_possible_path to true
                    found_a_possible_path = True
                # in neighbour not in visited
    ...
    # check if possible paths is empty
    if len(possible_paths) == 0:
        # if it is return an empty array
        return []
    
    # create an array of 0 for all the possible paths
    counts = [0 for i in possible_paths]
    # loop all the possible paths
    for i, path in enumerate(possible_paths):
        # assign room id to path
        room_id = path[-1]
        # assign room to map with roomid
        room = map[room_id]
        count = 0
        # loop over the all the keys in room
        for key in room:
            # if the key is equal to question mark
            if room[key] == '?':
                # increase count by 1
                count += 1
        # assign counts with the key of i to counts
        counts[i] = count
    # loop over the length of counts
    for i in range(len(counts)):
        # check if counts is equal to one
        if counts[i] == 1:
            # if it is return a possible path
            return possible_paths[i]
        # check if counts is equal 3
        if counts[i] == 3:
            # if it is return possible path
            return possible_paths[i]
    # return a random possible path
    return possible_paths[random.randint(0, len(possible_paths)-1)]


def directions(rooms_to_take, map):
    # declare an empty array
    directions = []
    # loop over all the rooms to take
    for i in range(len(rooms_to_take) - 1):
        # assign the current room to the map of rooms to take with index i
        current_room = map[rooms_to_take[i]]
        # loop over every key in current_room
        for key in current_room:
            # check if current current room is equal to room to take + 1
            if current_room[key] == rooms_to_take[i+1]:
                # if it is add key to directions
                directions.append(key)
                # and then break the function
                break
    # return directions
    return directions


# assign the current room of the player to the starting room
player.currentRoom = world.startingRoom
# create an empty array
traversalPath = []

# create a dictionayry with all
rooms_visited = {player.currentRoom.id: room_direction()}

# while rooms visited is smaller than roomgraph
while len(rooms_visited) < len(roomGraph):

    # run move_toward_end (pass in player, room_visited, traversePath)
    move_toward_end(player, rooms_visited, traversalPath)

    # call the find_path function and assign it to not_visited_rooms
    not_visited_room = find_path(player.currentRoom.id, rooms_visited)

    print('not visited room', not_visited_room)

    # call the directions function 
    unvisited_directions = directions(
        not_visited_room, rooms_visited)


    # loop over all the unvisited directions
